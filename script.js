...
// Tooltip placement helper: clamps tooltip so it doesn't go off-screen
function positionTooltip(selection, clientX, clientY, offsetX = 15, offsetY = 15) {
  const tooltip = selection.node();
  if (!tooltip) return;
  ...
  function renderChart(data) {
    ...
    svg.selectAll('.chart-dot')
      .on('mouseover', function(event, d) {
        ...
        positionTooltip(barTooltip, event.clientX, event.clientY, 15, 28);
...
document.addEventListener('DOMContentLoaded', async function () {
  // -------------------------------
  // Tab switching between Map & Scatter
  // -------------------------------
  const tabs = document.querySelectorAll('.viz-tab');
  const tabContents = document.querySelectorAll('.tab-content');
  ...
  // Shared color scale + label mapping (used by bar + globe)
  const colorScale = d3.scaleSequential()
    .domain([25, 60]) // nervous %
    .interpolator(d3.interpolateRgb('#4AADFF', '#FF4444'));
  ...
  // ========================================
  // SCATTER PLOT - EXTRA LARGE & INTERACTIVE
  // (unchanged, still Concerned vs Excited)
  // ========================================
  const scatterContainer = document.getElementById('scatterChart');
  if (scatterContainer) {
    const margin = { top: 40, right: 80, bottom: 80, left: 80 };
    const containerWidth = Math.min(1100, scatterContainer.clientWidth || window.innerWidth * 0.9);
    const width = containerWidth - margin.left - margin.right;
    const height = 500 - margin.top - margin.bottom;
    ...
    svg.append('g').selectAll('line.grid-line-x')
      .data(xScale.ticks(10)).enter().append('line')
      .attr('class', 'grid-line')
      .attr('x1', d => xScale(d)).attr('x2', d => xScale(d))
      .attr('y1', 0).attr('y2', height);

    svg.append('g').selectAll('line.grid-line-y')
      .data(yScale.ticks(7)).enter().append('line')
      .attr('class', 'grid-line')
    ...
    const dataArray = Object.entries(sentimentData)
    ...
    dots.on('mouseover', function (event, d) {
      ...
      svg.selectAll('.scatter-label')
        ...
        .style('font-weight', c => (c.country === d.country ? '800' : '400'))
        .style('font-size', c => (c.country === d.country ? '16px' : '13px'))
        .style('fill', c => (c.country === d.country ? '#fff' : '#71767B'));

      tooltip.html(`
        <div class="country-name">${d.country}</div>
        <div class="sentiment-values">
          <strong>Excited:</strong> ${d.excited.toFixed(1)}%<br/>
          <strong>Concerned:</strong> ${d.nervous.toFixed(1)}%<br/>
          <strong>Respondents:</strong> ${d.count.toLocaleString()}<br/>
          <div style="margin-top:8px; padding-top:8px; border-top:1px solid rgba(29,155,240,0.3);">
            <em style="color:#71767B;">Sentiment: ${d.excited > d.nervous ? 'More Excited' : 'More Nervous'}</em>
          </div>
    ...
    svg.selectAll('.scatter-label')
      ...
      .attr('x', d => {
        ...
        if (d.country === 'India' || d.country === 'Indonesia') return x + 15;
        if (d.country === 'Australia' || d.country === 'United Kingdom' || d.country === 'Portugal') return x - 15;
        return x > width / 2 ? x + 12 : x - 12;
      })
      .attr('y', d => yScale(d.nervous) + (d.country === 'India' ? 18 : 4))
      .attr('text-anchor', d => {
        if (['Australia', 'United Kingdom', 'Portugal'].includes(d.country)) return 'end';
        if (['India', 'Indonesia'].includes(d.country)) return 'start';
        return xScale(d.excited) > width / 2 ? 'start' : 'end';
      })
      .text(d => d.country)
      .style('fill', '#E7E9EA')
      .style('font-size', '14px')
  ...
  // ========================================
  // BAR CHART (respondents) + 3D GLOBE
  // ========================================
...
const mapContainer = document.getElementById('worldMap');
  if (mapContainer) {
    ...
    // Layout: left bar chart, right globe
    const layout = document.createElement('div');
  layout.className = 'sentiment-layout';
  layout.innerHTML = `
    <div class="sentiment-layout-title-row">
    </div>
    <div class="sentiment-bar-wrapper">
      <div id="sentimentBarChart"></div>
    </div>
    <div class="sentiment-globe-wrapper">
      <div id="sentimentGlobe"></div>
      <div class="sentiment-globe-footer">
        <div class="sentiment-legend-card">
          <div class="sentiment-legend-title">Concern Level</div>
          <div class="sentiment-legend-gradient"></div>
          <div class="sentiment-legend-labels">
          </div>
        </div>
      </div>
    </div>
  `;
    mapContainer.appendChild(layout);

    // Floating sentiment split overlay card (hidden until interaction)
   

    

    // ---------------------------
    // Horizontal bar chart: Sentiment gap (Concerned - Excited)
    // ---------------------------
    const barContainer = d3.select('#sentimentBarChart');

    const barMargin = { top: 60, right: 35, bottom: 50, left: 150 };
    const containerWidth = Math.min(
      800,
      (barContainer.node() ? barContainer.node().clientWidth : 0) || window.innerWidth * 0.65
    );
    const barWidth = containerWidth - barMargin.left - barMargin.right;

    const dataArray = Object.entries(sentimentData)
      .map(([country, data]) => {
        const gap = data.nervous - data.excited;
        return { country, ...data, gap };
      })
      .sort((a, b) => a.gap - b.gap); // most excited (negative) at top

    const barHeightInner = dataArray.length * 26;

    const svgBar = barContainer.append('svg')
      .attr('width', barWidth + barMargin.left + barMargin.right)
      .attr('height', barHeightInner + barMargin.top + barMargin.bottom)
      .attr('viewBox', `0 0 ${barWidth + barMargin.left + barMargin.right} ${barHeightInner + barMargin.top + barMargin.bottom}`)
      .style('background', '#000000')
      .style('border-radius', '16px');
    ...
    const maxAbsGap = d3.max(dataArray, d => Math.abs(d.gap));
    ...
    gBar.append('text')
      ...
      .style('font-size', '14px')
      .style('font-weight', '700')
      .text('Sentiment gap (Concerned − Excited)');
    ...
    gBar.append('line')
      ...
      .attr('x2', xScaleBar(0))
      .attr('y1', 0)
      .attr('y2', barHeightInner)
      .attr('stroke', 'rgba(231,233,234,0.3)')
      .attr('stroke-width', 1);
    ...
    gBar.append('g')
      .attr('class', 'bar-axis')
      .attr('transform', `translate(0,${barHeightInner})`)
      .call(
        d3.axisBottom(xScaleBar)
          .ticks(7)
          .tickFormat(d => `${d > 0 ? '+' : ''}${d}`)
      );
    ...
    gBar.append('text')
      .attr('x', barWidth / 2)
      .attr('y', barHeightInner + 40)
      .attr('text-anchor', 'middle')
      .style('fill', '#E7E9EA')
      .style('font-size', '11px')
      .text('Positive = more concerned · Negative = more excited');
...
let bars;
let flyToCountryOnGlobe = null;
let globeControls = null;

function handleBarClick(countryName) {
  if (!bars) return;

  // highlight bar
  bars.classed('bar-selected', d => d.country === countryName);

  // stop globe auto-rotation if we have controls
  if (globeControls) {
    globeControls.autoRotate = false;
  }

  // fly globe to that country if available
  if (flyToCountryOnGlobe) {
    flyToCountryOnGlobe(countryName);
  }
...
bars = gBar.selectAll('.sentiment-bar')
  .data(dataArray)
  .enter()
  .append('rect')
  ...
  .attr('x', d => xScaleBar(Math.min(0, d.gap)))
  .attr('width', d => Math.abs(xScaleBar(d.gap) - xScaleBar(0)))
  .attr('fill', d => colorScale(d.nervous))
  .attr('rx', 4)
  .attr('ry', 4)
  .on('mouseover', function (event, d) {
    d3.select(this)
      ...
      .transition()
      .duration(150)
      .attr('height', yScaleBar.bandwidth() + 3)
      .attr('y', yScaleBar(d.country) - 1.5);

    const gapText = `${d.gap >= 0 ? '+' : ''}${d.gap.toFixed(1)} pts`;

   

    barTooltip
      .html(`
        <div class="sentiment-tooltip-card">
          <div class="sentiment-tooltip-header">${d.country}</div>
          <div class="sentiment-tooltip-split">
            <div class="sentiment-tooltip-split-column">
              <div class="sentiment-tooltip-bar-outer">
                <div class="sentiment-tooltip-bar-inner excited"
                     style="height:${d.excited}%"></div>
              </div>
              <div>Excited</div>
              <div>${d.excited.toFixed(1)}%</div>
            </div>
            <div class="sentiment-tooltip-split-column">
              <div class="sentiment-tooltip-bar-outer">
                <div class="sentiment-tooltip-bar-inner nervous"
                     style="height:${d.nervous}%"></div>
              </div>
              <div>Concerned</div>
              <div>${d.nervous.toFixed(1)}%</div>
            </div>
          </div>
          <div style="font-size:11px;opacity:0.8;">
            Sentiment gap: <strong>${gapText}</strong><br/>
            Respondents: ${d.count.toLocaleString()}
          </div>
        </div>
      `)
      .classed('visible', true)
    ...
    if (typeof positionTooltip === 'function') {
      positionTooltip(barTooltip, event.clientX, event.clientY);
  ...
  .on('mouseout', function (event, d) {
    d3.select(this)
      ...
      .duration(150)
      .attr('height', yScaleBar.bandwidth())
      .attr('y', yScaleBar(d.country));

    barTooltip.style('opacity', 0).classed('visible', false);
  })
  .on('click', (event, d) => {
    handleBarClick(d.country);
  });


    // Select first country by default for the card
// ---------------------------
    // 3D Globe with sentiment
    // ---------------------------
async function initGlobe() {
  const globeContainer = document.getElementById('sentimentGlobe');
  if (!globeContainer) return;
  if (typeof Globe === 'undefined' || typeof topojson === 'undefined') {
    console.warn('Globe.gl or topojson not available');
    return;
  }
  ...
  try {
    ...
    const centroids = {};
    countries.forEach(f => {
      const norm = normalizeCountryName(f.properties.name);
      centroids[norm] = d3.geoCentroid(f); // [lng, lat]
    });
    ...
    const globe = Globe()(globeContainer)
      ...
      .showAtmosphere(true)
      .atmosphereColor('#4AADFF')
      .atmosphereAltitude(0.15)
      .polygonsData(countries)
      .polygonAltitude(f => {
        const norm = normalizeCountryName(f.properties.name);
        return sentimentData[norm] ? 0.06 : 0.01;
      })
      .polygonCapColor(f => {
        const norm = normalizeCountryName(f.properties.name);
        const data = sentimentData[norm];
        return data ? colorScale(data.nervous) : '#202327';
      })
      ...
      .polygonLabel(f => {
        ...
        return `
          <div class="sentiment-tooltip-card">
            <div class="sentiment-tooltip-header">${norm}</div>
            <div class="sentiment-tooltip-split">
              <div class="sentiment-tooltip-split-column">
                <div class="sentiment-tooltip-bar-outer">
                  <div class="sentiment-tooltip-bar-inner excited"
                       style="height:${data.excited}%"></div>
                </div>
                <div>Excited</div>
                <div>${data.excited.toFixed(1)}%</div>
              </div>
              <div class="sentiment-tooltip-split-column">
                <div class="sentiment-tooltip-bar-outer">
                  <div class="sentiment-tooltip-bar-inner nervous"
                       style="height:${data.nervous}%"></div>
                </div>
                <div>Concerned</div>
                <div>${data.nervous.toFixed(1)}%</div>
              </div>
            </div>
            <div style="font-size:11px;opacity:0.8;">
              Respondents: ${data.count.toLocaleString()}
            </div>
          </div>
        `;
      });
    ...
    // Gentle auto-rotation + interactive controls
    const controls = globe.controls();
    controls.autoRotate = true;
    controls.autoRotateSpeed = 0.45;
    controls.enableDamping = true;
    // expose controls so bar click can stop spinning
    globeControls = controls;

    globe.pointOfView({ lat: 20, lng: 0, altitude: 2.4 });

    // keep split card in sync on hover
    globe.onPolygonHover(f => {
      if (!f) return;
      const norm = normalizeCountryName(f.properties.name);
      if (sentimentData[norm]) {
        handleBarClick(norm);
      }
    });

    // stop spinning & update card when user clicks a country on the globe
    globe.onPolygonClick(f => {
      if (!f) return;
      const norm = normalizeCountryName(f.properties.name);
      if (!sentimentData[norm]) return;

      if (globeControls) {
        globeControls.autoRotate = false;
      }

      // reuse the same behavior as clicking a bar
      handleBarClick(norm);
    });

    // Expose fly-to function for bar clicks
    flyToCountryOnGlobe = (countryName) => {
      const centroid = centroids[countryName];
      if (!centroid) return;
      const [lng, lat] = centroid;
      globe.pointOfView({ lat, lng, altitude: 2.4 }, 1000);
    };
  } catch (e) {
    console.error('Error loading globe data:', e);
    globeContainer.innerHTML =
      '<p style="color:#E7E9EA;text-align:center;padding:2rem;font-size:16px;">Globe visualization could not be loaded.</p>';
  }
}


    // Init globe (don’t block the rest of the page)
    initGlobe();
  }
});
